...
async function handlePostImage(dato, sala, socket) {
	let { message, client, cantidadVectoresMenajes, cantidadMensajesHistorial, language, tone , isGpt, gpt, clientPerfil } = dato;
	...
	if (isGpt){
		console.log("isGpt");
		var promptTemplate = createPromptTemplateGpt(matches.length > 0, language, tone, gpt.prompt);
	}else{
		var promptTemplate = createPromptTemplate(matches.length > 0, language, tone);
	}
...
function createPromptTemplateGpt(hasMatches, language, tone, dynamicPrompt) {
	const templateWithDocs = `
	- Your behavior and responses are guided by the following instructions:
	- ${dynamicPrompt}
	- You will be given INPUT, CHATHISTORY, MEMORY, and RELEVANTDOCS.
	- INPUT corresponds to the message the user is sending, which can be a question or a request.
	- CHATHISTORY is a memory of past messages related to the user's message.
